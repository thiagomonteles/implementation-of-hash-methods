...
#include <stdlib.h>
#include <string.h>
#include "hashingfunctions_collision_zone.h"
/*
	* Esse programa ira ultilizar o metodo de DOBRA para criar o HASH e armazenar
	ultilizando o método de zona de colisão.
 */
...
int zona_de_colisao = sizeof(Registro)*M;
...
void gravar_colisao(int hash,FILE*datahash, Registro * RegAux){
	/* Essa função irá gravar as colisões na zona de colisao*/
...
void procurar_registro(int id){
	...
	if(Raux.id == id) printPessoa(&Raux);
		...
		// se o primeiro registro não for o que eu procuro, eu procuro lineamente o id desejado na zona de colisao
	
		//vou pra zona de colisao
		fseek(datahash,zona_de_colisao,SEEK_SET);
		numero_de_operacoes++;
		while(!feof(datahash) ){ //Enquanto não chegar no final do arquivo
			...
			fseek(datahash,tamR,SEEK_CUR);//pulo um registro
			numero_de_operacoes++;
		}
	}
	fclose(datahash);
...
void deletar_registro(int id){
	...
	Caso contrario começa a buscar o lineamente o ID na zona de colisao  */
	Registro Raux,RegColisoes;
	int tamR = sizeof(Registro);
	int hash = hashDobra(id,8,4);
	int local_atual;
	...
	if(Raux.id == id){ 
		...
		while(!feof(datahash)){	
			...
			if(feof(datahash)){ //se não existe um proximo
