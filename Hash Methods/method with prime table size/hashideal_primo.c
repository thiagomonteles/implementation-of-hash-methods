...
#include <stdlib.h>
#include <string.h>
#include "hashingfunctions_primo.h"

/*
	* Usando o metódo de encontrar o proximo primo da quantidande incial de elementos na tabela para usar como
	 M, e usando esse M para criar o Hash ultilizando o método do resto 
...
void inserirregistro(Registro * registro, Pessoa *pessoa){
	...
	if(registro->status != 'O'){
		...
		registro->per=pessoa;
		registro->status='O';
		registro->quant_colisoes = 0;
	}
	else if(registro->status == 'O'){
...
void freetabela(Registro * tabela,int M){
	/* Função responsavel por liberar a memória ocupada pela tabela */
	int i;
	for (i = 0; i < M; i++){
		if(tabela[i].status != 'O')continue;
		else{
			free(tabela[i].per);
		}
	}
...
void processar_arquivo(Registro *tabela,int M){
	...
	while((i<M) &&(fgets(linha, sizeof linha, arq) != NULL)){
		...
		inserirregistro(&tabela[hash],pessoa);
		i++;
	}
...
void procurar_registro(int id,Registro *tabela,int M){
...
void verificar_espacos_entre_registros(Registro * tabela,int M){
...
int main(int argc, char const *argv[]){
	//Achando o proximo numero primo de M para ultilizar na tabela
	int M = achaProximoPrimo(10000);
	...
	verificar_espacos_entre_registros( tabela, M);
	freetabela(tabela,M);
	return 0;
}
